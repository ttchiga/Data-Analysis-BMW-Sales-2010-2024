...
format: 
  dashboard:
    nav-buttons: [github]
    github: https://github.com/ttchiga/Data-Analysis-BMW-Sales-2010-2024.git
fig-width: 10
fig-asp: 0.3
---
...
df<- df%>%
  ...
  mutate(revenue = sales_volume*price_usd,
      ...
      model %in% c("X1","X3","X5","X6") ~"SUV",
      model %in% c("i8", "i3") ~ "Electric/Hybrid",
      model %in% c("3 Series", "5 Series",  "7 Series" ) ~ "Sedan",
      model %in% c("M3", "M5") ~ "Performance"),
      engine_size = case_when(
        engine_size_l >=1.0 & engine_size_l<=2.0 ~"Small Engine",
        engine_size_l > 2.0 & engine_size_l<=3.0 ~"Medium Engine",
        engine_size_l > 3.0 & engine_size_l<=5 ~ "Large Engine"
      ),
      num_engine_size = case_when(
        engine_size_l >=1.0 & engine_size_l<=2.0 ~1,
        engine_size_l > 2.0 & engine_size_l<=3.0 ~2,
        engine_size_l > 3.0 & engine_size_l<=5 ~ 3
      ),
      fuel = case_when(
        fuel_type =="Hybrid" ~0,
...
format_number <- function(x) {
  case_when(
    abs(x) >= 1e12 ~ paste0(round(x/1e12,2), "T"),
    abs(x) >= 1e6 ~ paste0(round(x/1e6, 2), "M"),
    abs(x) >= 1e3 ~ paste0(round(x/1e3, 2), "K"),
    TRUE ~ as.character(x)
  )
}
...
find_strong_correlations <- function(cor_matrix, threshold = 0.7) {
  # Set diagonal to NA to avoid self-correlation
  diag(cor_matrix) <- NA
  
  strong_pairs <- which(abs(cor_matrix) > threshold, arr.ind = TRUE)
  
  results <- data.frame(
    Region1 = rownames(cor_matrix)[strong_pairs[, 1]],
    Region2 = colnames(cor_matrix)[strong_pairs[, 2]],
    Correlation = cor_matrix[strong_pairs]
  )
  
  # Remove duplicates and sort by absolute correlation
  results <- results[results$Region1 != results$Region2, ]
...
ggplot(price_summary, aes(x = date, y = avg_price, group = region)) +
  ...
  scale_x_continuous(limits = c(2010, 2026))  # Extra space for labels
...
#| title: Annual Sales Per Region 

 df |> 
  group_by(region,date)|>
  summarise(annual_sales =sum(sales_volume))|>
  ungroup()|>
  group_by(region)|>
  mutate( total_sales= sum(annual_sales, na.rm=TRUE))|>
  ungroup()|>
  mutate(date=lubridate::year(date),
         annual_sales = format_number(annual_sales),
         total_sales = format_number(total_sales))|>
  pivot_wider(names_from = date, values_from = annual_sales)|>
  relocate(total_sales, .after = last_col())|>
  kable("html", caption = "Annual Sales per Region") %>%
...
df|> 
  ...
  labs(title="Revenue Before and After Covid",
    x="",y="Revenue ")+
  theme_minimal()+
   theme(
    axis.text.x = element_text(angle = 45, hjust = 1),
...
#| title: Changes in Market Tastes and Preferences 

df|>
  group_by(date,model_family,region)|>
  summarize(annual_sales = sum(sales_volume),
            avg_price = median(price_usd),
            .groups = "drop")|>
  ungroup()|>
  ggplot(aes(date,annual_sales, group=model_family))+
  geom_line(aes(color=model_family),size=1.1)+
  geom_point(color="steelblue1", alpha =.5)+
  facet_wrap(.~region)+
  scale_color_manual(
    values = c( "SUV"="skyblue4",
     "Electric/Hybrid"="skyblue1",
     "Sedan"="deepskyblue",
       "Performance"="gray80"))+
  theme_minimal()+
  labs(y="Annual Sales",x="",
       subtitle = "Changes in Model Composition in Different Markets")+
  scale_y_continuous(labels=format_number)+
...
df_growth_cor <- df %>%
  ...
  filter(date!=as.Date("2010-01-01"))|>
    select (date,region, sales_yoy_growth)|>
  pivot_wider(names_from = region, values_from = sales_yoy_growth)


# Calculate correlation matrix between regions
# Remove the Year column for correlation calculation
region_correlations <- cor(df_growth_cor[, -1]) # Exclude Year column


# Find strongly correlated regions
strong_corrs <- find_strong_correlations(region_correlations, threshold = 0.35)
...
df_kcluster <- df|>
  select(price_usd, engine_size_l, fuel, mileage_km, age)|>
  scale()|>
  as.data.frame()%>%
  sample_n(min(5000, nrow(.)))

fviz_nbclust(df_kcluster, kmeans, method = "wss") +
  labs(title = "Elbow Method for Optimal k")


kmeans_res <- kmeans(df_kcluster, centers = 4, nstart = 25)

fviz_cluster(kmeans_res, data = df_kcluster,
             geom = "point",
             ellipse.type = "convex",
             repel = TRUE,
